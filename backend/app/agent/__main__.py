"""Agent smoke test from the terminal — watch the loop think in tools.
    ...
    python -m app.agent --agent langgraph --model deepseek-api "..."
...
result = get_agent(agent_kind).run(question, model=model_name, max_steps=max_steps)
